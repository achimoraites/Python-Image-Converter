...
import os
from raw_image_converter.utils import check_extension, convert_file, convert_raw, image_not_exists, delete_directory
import argparse
...
def main():
    ...
    parser.add_argument('-s', "--src", dest = "src_dir",help='specify the source directory!', required=True) 
    parser.add_argument("-t","--tgt", dest = "tgt_dir", help="specify the target directory!", default="converted")
    parser.add_argument('-e',"--ext", dest = "ext", default=".jpg", choices=['.jpg', '.png'],
                      help='the image format to be used for the converted images.')
    parser.add_argument("--delete-source-directory", action="store_true", dest="delete_source_directory", default=False, help="Delete the source directory after the convesion")
